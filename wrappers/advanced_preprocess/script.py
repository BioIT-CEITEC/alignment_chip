######################################
# wrapper for rule: advanced_preprocess
######################################
import subprocess
import os
...
fastq_c1 = snakemake.output.fastq[0]
if snakemake.params.paired:
    is_paired = True
    fastq_r2 = snakemake.input.fastq[1]
    fastq_c2 = snakemake.output.fastq[1]
...
if int(snakemake.params.trim_left1) > 0 or int(snakemake.params.trim_right1) > 0:
    ...
    command = "$(which time) seqtk trimfq "+extra_flags_seqtk+" "+fastq_r1+" 2>> "+log_filename+\
              " | gzip -c > "+fastq_r1.replace(".gz",".trim.gz")+" 2>> "+log_filename
    f = open(log_filename, 'at')
    f.write("## COMMAND: "+command+"\n")
    f.close()
    shell(command)

if is_paired and (int(snakemake.params.trim_left2) > 0 or int(snakemake.params.trim_right2) > 0):
    ...
    command = "$(which time) seqtk trimfq "+extra_flags_seqtk+" "+fastq_r2+" 2>> "+log_filename+\
              " | gzip -c > "+fastq_r2.replace(".gz",".trim.gz")+" 2>> "+log_filename
    f = open(log_filename, 'at')
    f.write("## COMMAND: "+command+"\n")
    f.close()
    shell(command)
...
command = "mkdir -p " + os.path.dirname(snakemake.params.trim_stats)+" >> "+log_filename+" 2>&1"
f = open(log_filename, 'at')
f.write("## COMMAND: " + command + "\n")
f.close()
shell(command)
...
if is_paired:
    ...
    command = "mkdir -p " + os.path.dirname(snakemake.params.r1u)+" >> "+log_filename+" 2>&1"
    f = open(log_filename, 'at')
    f.write("## COMMAND: " + command + "\n")
    f.close()
    shell(command)

    command = "$(which time) trimmomatic PE -threads "+str(snakemake.threads)+\
              " "+snakemake.params.phred+" "+input_r1+" "+input_r2+" "+fastq_c1+"\
              "+snakemake.params.r1u+" "+fastq_c2+" "+snakemake.params.r2u+" CROP:"+str(snakemake.params.crop)+\
              " LEADING:"+str(snakemake.params.leading)+" TRAILING:"+str(snakemake.params.trailing)+\
              " SLIDINGWINDOW:"+str(snakemake.params.slid_w_1)+":"+str(snakemake.params.slid_w_2)+\
              " "+adaptors+" MINLEN:"+str(snakemake.params.minlen)+"\
    ...
    command = "$(which time) trimmomatic SE -threads " + str(snakemake.threads) +\
              " " + snakemake.params.phred + " " + input_r1 + " " + fastq_c1 + " " + adaptors + \
              " CROP:" + str(snakemake.params.crop) + " LEADING:" + str(snakemake.params.leading) + \
              " TRAILING:" + str(snakemake.params.trailing) + \
              " SLIDINGWINDOW:" + str(snakemake.params.slid_w_1) + ":" + str(snakemake.params.slid_w_2) + \
              " MINLEN:" + str(snakemake.params.minlen) + " " + trim_left + \
              " -Xmx" + str(snakemake.resources.mem) + "g >> " + snakemake.params.trim_stats + " 2>&1 "
    f = open(log_filename, 'at')
    f.write("## COMMAND: " + command + "\n")
    f.close()
    shell(command)
